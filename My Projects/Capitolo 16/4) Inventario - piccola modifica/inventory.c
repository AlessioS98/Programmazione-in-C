...
void insert(void)
{
    ...
    printf("Inserisci il prezzo del componente: €");
...
void update(void)
{
    ...
    if(i>=0)
    {
        ...
        scanf("%d", &change);
        printf("Aggiungi prezzo: ");
        scanf("%f", &changeprice);
        inventory[i].price+=changeprice;
...
void print(void)
{
    ...
    int minimo;
    int i, j, k, indicemin;
    printf("Numero componente     Nome componente     Quantita' componente     Prezzo componente\n");
    j=0;
    while(j<num_parts)
    {
